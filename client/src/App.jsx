...
export default function App() {
  ...
  const handleDeleteProduct = async (id) => {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setProducts((prev) => prev.filter((p) => !isProductMatch(p, id) && !isProductMatch(p, numericId)));
        try {
          const menuRes = await fetch('/api/menu').then((r) => r.json());
          if (menuRes && menuRes.success && Array.isArray(menuRes.products)) {
